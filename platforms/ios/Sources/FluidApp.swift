...
@main
struct FluidApp: App {
    /// Before the first view reads a stored choice, so a phone that
    /// stored the old three booleans keeps the look it was left on.
    init() { Settings.migrate() }
...
struct ContentView: View {
    ...
    @AppStorage(Settings.lookKey) private var look = Look.glass.rawValue
    ...
    var body: some View {
        ...
        .sheet(isPresented: $menuShown, onDismiss: { buttonShown = false }) {
            MenuSheet(
                particleScale: $particleScale, look: $look,
                flatColour: $flatColour, highColour: $highColour, lens: $lens,
    ...
    private func setLook() {
        driver.setLook(
            look: Look(rawValue: look) ?? .glass,
            colour: FluidVec3(hex: flatColour),
            high: FluidVec3(hex: highColour),
            lens: UInt32(lens))
    }
}
